...
for c in range(len(chosen_list)):
    if chosen_list[c][5] == "filler":
        ...
        chosen_list[c].append(chosen_cb_list[cb_list_index])
        cb_list_index = cb_list_index + 1
...
#Terms and consent
dis_txt("This study is ran by Viktor Nielsen, 3rd semester bachelor student at Cognitive Science at Aarhus University. To continue participation, you must consent to me recording your data from the experiment. I don't collect any personal data, so your data is completely anonymized. You will be assigned a unique random ID, which neither of us will know. This also means that you won't be able to withdraw consent for me to use your data after the experiment. To consent to these terms and continue the experiment, please press space")
event.waitKeys(keyList=["space"])
...
for i in range(len(chosen_list)):
    ...
    for j in range(len(wlist)): 
        ...
        if j == sentence[4]-1 and key_cond == "out_of_key" and "filler" not in sentence[5]:
            chord = chords[0]
        ...
        elif j > 0 and j == sentence[4]: #this fix has been added after running experiments. Post_crit was being assigned to first segment in some sentences
        ...
        mus_experience.append(gold_msi)
        print(sentence_type[i])
